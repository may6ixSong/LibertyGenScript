...
쓰든(그 PDK가 이미 같은 걸 정의해뒀든 아니든) 항상 유효하도록 한다. PDK가 동일한
이름/그룹으로 이미 정의해뒀어도 완전히 같은 내용의 재정의라 Liberty 컴파일러가 보통
그대로 허용한다.

**이 목록은 block4_writer.py / block5_writer.py가 실제로 `{process_prefix}_*`로 쓰는
모든 줄과 정확히 일치해야 한다** - 두 파일 중 하나에서 `{process_prefix}_` 로 시작하는
줄을 추가/삭제/이름변경하면 반드시 이 목록도 같이 맞춰야 한다.
"""
...
def write_process_prefix_defines(f_out, process_prefix: str) -> None:
    """
    ...
    for group_suffix, parent_group in _CUSTOM_GROUPS:
        f_out.write(f"{INDENT_1}define_group({process_prefix}_{group_suffix}, {parent_group}) ;\n")
    for attr_suffix, group, value_type in _ATTRIBUTES:
        group_name = group if group in ("cell", "pin") else f"{process_prefix}_{group}"
        f_out.write(f"{INDENT_1}define({process_prefix}_{attr_suffix}, {group_name}, {value_type}) ;\n")
    f_out.write("\n")
